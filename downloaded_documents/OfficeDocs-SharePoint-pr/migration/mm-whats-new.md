...
- **Refind Identity Mapping.** Enhances the accuracy of automatic identity mapping in cloud migration scenarios.

## November 2025

- **PowerShell for File Share migration**. The PowerShell cmdlet is now available for you to manage file share migration tasks running in Migration Manager, and download scan and migration reports. [Learn more](/sharepointmigration/mm-powershell).

- **Custom Azure storage for File Share migration**. You can configure your own Azure storage as the temporary storage for content and manifest during migration by specifying it in the agent configuration file. [Learn more](/sharepointmigration/mm-setup-clients).

## July 2025

- **Delta sync for File Share migration**. File Share migration now supports delta sync, significantly improving the speed of incremental migrations. [Learn more](/sharepointmigration/mm-fileshare-copy-to-migrations).

## June 2025

- **Migration Manager Lite**. Migration Manager Lite is a simplified version of Migration Manager designed to help SMB customers easily migrate their content from Google Drive to Microsoft 365. It's enabled by default for SMB customers who meet the required conditions.

## May 2025

- **Simplified GWS migration**. Simplified GWS migration brings together multiple migration features for Google customers into a unified entry point in the M365 admin center, including Google Drive migration for files and Gmail migration for emails.
...
## August 2024
- **Revamped "Copy to Migrations" panel.** The panel was redesigned, enabling destination uploads and task-level customization while sending scanned tasks to the Migrations tab. This redesign improves migration flow efficiency.
- **Unified Filter experience.** Filter functionality was standardized across the Scans and Migrations tabs for a consistent user experience.
...
- **Date completed column**. A date completed column was added so customers know the exact date when the task completed processing.
