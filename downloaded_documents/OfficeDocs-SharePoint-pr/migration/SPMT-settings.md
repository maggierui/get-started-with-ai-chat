...
|Setting                  |Description |
|:------------------------|:-----------|
|Use custom Azure Storage |If you wish to use your own Azure Storage, set this value to **On**. <br/>If you turn on **Upload content with Entra Authentication**, provide **Azure storage URL** and assign the "Storage Blob Data Contributor" role to a security principal in your storage account at the Azure portal. If you turn off Upload content with Entra Authentication, provide **Azure storage URL** and **account key**. <br/>If you turn off **Consume content with Entra Authentication**, assign "Storage Blob Data Contributor" role to security principal "Office 365 SharePoint Online". <br/>Select if you want to enable encryption, and if you want temporary working files to be deleted when migration is complete. |

> [!NOTE]
> **Upload content with Entra Authentication** and **Consume content with Entra Authentication** options are available in SPMT build 4.3.130.0 or newer. 
> The security principal that accesses custom Azure storage can be
> o   __User account__ which is used to connect the destination SharePoint
> o   **R**__egistered application__ when certificate-based authentication is configured to access the destination SharePoint
