...
ms.custom:
ms.assetid:
description: "Learn about how workflows are structured when migrating from SharePoint Server to Power Automate using SPMT."
---
...
The following chart shows the first action in a flow migrated from an **out-of-the-box (OOTB)** approval workflow (manual start) and the UI form when a user starts the flow. It lists the mapping of five user inputs between original workflow and converted flow.
