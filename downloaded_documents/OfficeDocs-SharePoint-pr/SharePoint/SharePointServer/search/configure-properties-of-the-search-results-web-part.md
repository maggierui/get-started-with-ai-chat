...
5. On the **BASICS** tab, do one of the following: 
  ...
  - To define your query by using Keyword Query Language (KeyQL), select options as described in the following list:
    ...
    - **Query text**
      ...
      By default, the query variable {searchboxquery} is defined for this field. You can change the query text by using KeyQL. For more information about KeyQL, see [Keyword Query Language (KeyQL) syntax reference](/sharepoint/dev/general-development/keyword-query-language-kql-syntax-reference). Alternatively you can use the **Keyword filter** and **Property filter** lists to build the query. 
